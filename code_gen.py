...
def declare_arr(param=0):
    ...
    symbol_table.SYMBOL_TABLE.append(symbol_table.Symbol(id, 'array', addr, attr= size))
...
def declare_func(param=None):
    ...
    symbol_table.SYMBOL_TABLE.append(symbol_table.Symbol(SEMANTIC_STACK[-4], 'func', attr, attr= attr))
    for i in range(4): 
        SEMANTIC_STACK.pop()
    # TODO: handle main
...
def pid(id):
    SEMANTIC_STACK.append(symbol_table.find_addr(id.value))
...
def break_op(param= None):
    ...
    if len(BREAK_LIST) == 0:
        SEMANTIC_ERROR.append(f"#{param.line}: Semantic Error! No 'repeat ... until' found for 'break'")
        return
